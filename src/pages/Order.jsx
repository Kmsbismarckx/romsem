...
function Order() {
  ...
  const [shippingType, setShippingType] = useState({ courier: '_active', selfPickup: '' });
  const [paymentType, setPaymentType] = useState({ cash: '_active', card: '' });
  const [shippingTime, setShippingTime] = useState({ now: '_active', later: '' });

  const totalPrice = useSelector(selectTotalPrice);

  const shippingTypeHandler = () => {
    if (shippingType.courier === '_active') {
      setShippingType({ ...shippingType, courier: '', selfPickup: '_active' });
    } else {
      setShippingType({ ...shippingType, courier: '_active', selfPickup: '' });
    }
  };
  const paymentTypeHandler = () => {
    if (paymentType.cash === '_active') {
      setPaymentType({ ...paymentType, cash: '', card: '_active' });
    } else {
      setPaymentType({ ...paymentType, cash: '_active', card: '' });
    }
  };
  const shippingTimeHandler = () => {
    if (shippingTime.now === '_active') {
      setShippingTime({ ...shippingType, now: '', later: '_active' });
    } else {
      setShippingTime({ ...shippingType, now: '_active', later: '' });
    }
  };
    ...
    <div className="order ">
      ...
      <div className="order__form">
        ...
        <div className="form__type-of-shipping form__toggle order__form_border-radius order__form_width">
          <div
            className={`order__form__item form__toggle__item form__toggle__item${shippingType.courier} form__courier`}
            onClick={shippingTypeHandler}
          >
            Курьером
          </div>
          <div
            className={`order__form__item form__toggle__item form__toggle__item${shippingType.selfPickup}  form__self-pickup`}
            onClick={shippingTypeHandler}
          >
            Самовывоз
          </div>
        ...
        <div className="form__type-of-payment form__toggle order__form_border-radius order__form_width ">
          <div
            className={`order__form__item form__toggle__item form__toggle__item${paymentType.cash} form__cash`}
          ...
          >
            ...
            >
              <path
                className={`order__svg${paymentType.cash}`}
          ...
          <div
            className={`order__form__item form__toggle__item form__toggle__item${paymentType.card} form__card`}
            onClick={paymentTypeHandler}
          >
            <svg
              width="22"
            ...
            >
              <g clipPath="url(#clip0_25_525)">
                <path
                  className={`order__svg${paymentType.card}`}
        ...
        <div className="form__shipping-time form__toggle order__form_border-radius order__form_width ">
          <div
            className={`order__form__item form__toggle__item form__toggle__item${shippingTime.now} form__shipping-time-now`}
            onClick={shippingTimeHandler}
          >
            На сейчас
          </div>
          <div
            className={`order__form__item form__toggle__item form__toggle__item${shippingTime.later} form__shipping-time-later`}
            onClick={shippingTimeHandler}
          >
            На время
          </div>
